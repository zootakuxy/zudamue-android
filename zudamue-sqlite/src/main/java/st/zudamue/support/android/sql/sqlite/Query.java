...
public class Query extends BaseSQLExecutable {
    ...
    public @NonNull List<SQLRow> catchAllResult( OnQueryResult onCatchResult ) {
        ...
        if(hasRow()) {
            ...
            do{
                SQLRow row = Query.catchResultRow( loopCursor, index );
                list.add( row );
     ...
     */
    public static SQLRow catchResultRow( Cursor cursor, Map<String, SQLiteRow.HeaderCell> indexRow ) {
    ...
    public SQLRow nextRow() {
        SQLRow row = Query.catchResultRow(this.cursor, getIndexer());
